...
try {
  ...
  for (const angle of ANGLES) {
    const url = `${BASE}?model=${encodeURIComponent(model)}&angle=${angle}&title=${encodeURIComponent(title)}${sea != null ? `&sea=${sea}` : ''}${build != null ? `&build=${build}` : ''}${lat != null && lon != null ? `&lat=${lat}&lon=${lon}` : ''}`;
